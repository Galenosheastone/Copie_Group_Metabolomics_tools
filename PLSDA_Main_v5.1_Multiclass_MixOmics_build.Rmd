...
mSet <- Read.TextData(mSet, "EXAMPLE_DATA3.csv", "rowu", "disc")
...
perf.plsda <- perf(plsda.res, validation = c("Mfold", "loo", "Q2"), folds = 5, 
...
pls.res <- splsda(X, Y, ncomp = 5, mode = "classic", max.iter = 100)


val <- perf.spls(s.plsda.res, criterion = c("all"), folds = 5)
val
plot(val)
...
plsda.plot <- ggplot(new.plsda.data, aes(x= comp1, y= comp2, color = Group)) +
              ...
              scale_color_manual(name="Variety", values=c("darkorange", "blue","springgreen2","cyan")) +
              ...
              scale_fill_manual(values=c("darkorange", "blue", "springgreen2", "cyan")) 
